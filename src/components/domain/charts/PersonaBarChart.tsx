...
const MOCK_TOP_PLANS = [
  ...
  { name: "프리미엄 요금제", users: 2124, percent: 35 },
  { name: "베이직 요금제", users: 1234, percent: 25 },
];
...
function CustomTooltip({ active }: TooltipProps<number, string>) {
  ...
  return (
    <div className="w-64 rounded-md border border-neutral-200 bg-white p-4 text-sm shadow-md">
      <p className="mb-1 font-semibold text-neutral-800">데이터 절약 유저</p>

      <p className="mb-3 text-xs text-neutral-400">데이터 절약 유저의 요금제 Top 3</p>
      ...
      <div className="flex justify-between">
        ...
        <span className="font-semibold text-neutral-800">
          {MOCK_TOTAL_USERS.toLocaleString()}명
        </span>
      </div>
    </div>
  );
