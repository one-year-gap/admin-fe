...
export function getColumns(opts: {
  ...
  onBulkCoupon: () => void;
  onSingleCoupon: (id: string) => void;
}): ColumnDef<ChurnRiskRow>[] {
  const { selectedCount, onBulkCoupon } = opts;
        ...
        return (
          <div className="flex items-center justify-center">
            <button
              ...
              onClick={() => opts.onSingleCoupon(row.original.id)}>
              쿠폰 발급
            </button>
          </div>
        );
